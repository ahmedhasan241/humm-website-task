<template>
  <div class="container my-4">
    <Head>
      <Title>
        {{ data?.shows[0].all_episodes[0].translations[0].title }} - همم |
        همم</Title
      >
    </Head>
    <iframe
      class="embed-responsive-item video-contain"
      :src="`https://www.youtube.com/embed/${data?.shows[0].all_episodes[0].video}`"
    ...
    <div class="slug-contain mt-5">
      <div class="row">
        <div class="col-12 col-md-8">
          <NuxtLink
            :to="`/shows/${route.params.category}`"
            class="category-link"
            type="button"
          >
            {{ data?.shows[0].translations[0].title }}
          </NuxtLink>
          <h1 class="mt-3 mb-4">
            {{ data?.shows[0].all_episodes[0].translations[0].title }}
          </h1>
          <div
            class="mb-5"
            v-html="data?.shows[0].all_episodes[0].translations[0].content"
          ></div>
            ...
            <div class="d-flex">
              <div class="d-flex align-items-center my-1">
                ...
                <p class="my-auto mx-1" style="font-size: 15px">
                  {{ formatDate(data?.shows[0].all_episodes[0].date_created) }}
                </p>
              </div>
            </div>
...
const { data } = await useAsyncGql({
  operation: "SingleShow",
  variables: { category: route.params.category, episode: route.params.slug },
});
